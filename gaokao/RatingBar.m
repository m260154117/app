...
//
//  Created by MiaoLizhuang on 15/12/15.
...
-(void)setImageDeselected:(NSString *)deselectedName halfSelected:(NSString *)halfSelectedName fullSelected:(NSString *)fullSelectedName andDelegate:(id<RatingBarDelegate>)delegate{
    ...
    [_s2 setFrame:CGRectMake(width+10,     0, width, height)];
    [_s3 setFrame:CGRectMake(2 * (width+10), 0, width, height)];
    [_s4 setFrame:CGRectMake(3 * (width+10), 0, width, height)];
    [_s5 setFrame:CGRectMake(4 * (width+10), 0, width, height)];
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    int newRating = (int) (point.x / (width+10)) + 1;
    if (newRating > 5)
        return;
